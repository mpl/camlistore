...
type FileConstraint struct {
	...
	// ParentDir is run against all parent directories of the file until
	// one matches.
...
// DirConstraint enables searching for directories. Contains and
// RecursiveContains are mutually exclusive.
type DirConstraint struct {
	...
	// ParentDir is run against all parent directories of the directory
	// until one matches.
	ParentDir *DirConstraint `json:"parentDir,omitempty"`
	...
	TopFileCount *IntConstraint `json:"topFileCount,omitempty"` // not recursive

	// RecursiveContains is like Contains, but applied to all the descendants
	// of the directory.
	RecursiveContains *Constraint `json:"contains,omitempty"`

	// Contains should have a BlobPrefix, or a *FileConstraint, or a *DirConstraint,
	// or a *LogicalConstraint combination of the aforementioned. It is run against
	// the children of the directory, in a non-recursive manner.
...
func (c *DirConstraint) blobMatches(ctx context.Context, s *search, br blob.Ref, bm camtypes.BlobMeta) (bool, error) {
	...
	if pfx := c.BlobRefPrefix; pfx != "" {
		if !strings.HasPrefix(br.String(), pfx) {
			return false, nil
		}
	}
	...
	if cc != nil {
		...
		if cc.BlobRefPrefix != "" {
			containsMatch, err = c.childrenMatch(ctx, s, children, func(ctx context.Context, s *search, child blob.Ref, bm camtypes.BlobMeta) (bool, error) {
				return strings.HasPrefix(child.String(), cc.BlobRefPrefix), nil
			})
		} else {
			...
			containsMatch, err = c.childrenMatch(ctx, s, children, cc.fileOrDirOrLogicalMatches)
		}
		if err != nil {
			return false, err
		}
	...
	if !containsMatch && recursive {
		match, err := c.childrenMatch(ctx, s, children, c.blobMatches)
		if err != nil {
			return false, err
		}
		if !match {
...
// childrenMatch runs matcher against each child and returns true as soon as
// there is a match, of false if none of them is a match.
func (c *DirConstraint) childrenMatch(ctx context.Context, s *search, children map[blob.Ref]struct{},
	matcher func(context.Context, *search, blob.Ref, camtypes.BlobMeta) (bool, error)) (bool, error) {
	// TODO(mpl): do them concurrently.
