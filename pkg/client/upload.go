...
type FileUploadOptions struct {
	// If non nil, FileInfo is used to populate the schema of the file blob.
	FileInfo os.FileInfo
	// If valid, WholeRef is assumed to represent the digest of the uploaded file
	// contents, which allows UploadFile to skip computing the digest (needed to check if
	// the contents are already on the server).
	WholeRef blob.Ref
}
...
// N.B: this method is still a work in progress, and might change to accomodate
// the needs of camput file.
func (cl *Client) UploadFile(filename string, contents io.Reader, fOpts *FileUploadOptions) (blob.Ref, error) {
	fileMap := schema.NewFileMap(filename)
	if fOpts != nil && fOpts.FileInfo != nil {
		fileMap = schema.NewCommonFileMap(filename, fOpts.FileInfo)
		modTime := fOpts.FileInfo.ModTime()
		if !modTime.IsZero() {
	...
	if fOpts != nil && fOpts.WholeRef.Valid() {
		wholeRef = fOpts.WholeRef
	} else {
		var buf bytes.Buffer
		var err error
