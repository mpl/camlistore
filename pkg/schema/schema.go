...
type superset struct {
	...
	// TODO(mpl): Add a MembersParts (that we would use instead of Members when a large dir), instead of LargeDir?
	LargeDir bool `json:"largeDir"` // if true, Members are sub static-sets, instead of directly the children.
...
// blob, they are spread (recursively, if needed) onto sub static-sets. In this
// case, these subsets are the ones set as members of this builder. All the created
...
func (bb *Builder) SetStaticSetMembers(members []blob.Ref) []*Blob {
	if bb.Type() != "static-set" {
		panic("called AddStaticSetMembers on non static-set")
	}

	if len(members) <= maxStaticSetMembers {
		children := make([]string, 0, len(members))
		for _, ref := range members {
			children = append(children, ref.String())
		}
		bb.m["members"] = children
		return nil
	}
	...
	fakeChildren := make([]string, 0, len(subsets))
	for _, b := range subsets {
		fakeChildren = append(fakeChildren, b.BlobRef().String())
	}
	bb.m["members"] = fakeChildren
	bb.m["largeDir"] = true
	return allSubsets
}
