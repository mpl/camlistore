...
func (s *storage) checkLargeIntegrity() (int, error) {
	...
	if err := blobserver.EnumerateAllFrom(context.TODO(), s.large, "", func(sb blob.SizedRef) error {
	...
	for k, _ := range inMeta {
		if _, ok := inLarge[k]; !ok {
			// TODO(mpl): running with recovery won't actually fix that. Should -recovery
			// remove stale index entries then? or add another recovery mode that blows up the
			// meta before rebuilding it?
			return FullRecovery, fmt.Errorf("packed blobRef %v in blobpacked meta does not seem to actually exist in large storage", k)
		}
	}
