...
func (h *handler) signAndSend(json string) error {
	...
	resp, err := h.cl.Post(h.server+"sighelper/camli/sig/sign", "application/x-www-form-urlencoded",
		strings.NewReader("json="+json))
	if err != nil {
		return fmt.Errorf("could not sign claim %v: %v", json, err)
	}
	defer resp.Body.Close()
	scl, err := ioutil.ReadAll(resp.Body)
	if err != nil {
...
func (h *handler) newPermanode() (blob.Ref, error) {
	...
	resp, err := h.cl.Post(h.server+"sighelper/camli/sig/sign", "application/x-www-form-urlencoded",
		strings.NewReader("json="+upn))
	if err != nil {
		return pn, fmt.Errorf("could not sign permanode: %v", err)
	}
	defer resp.Body.Close()
	spn, err := ioutil.ReadAll(resp.Body)
	if err != nil {
...
func (h *handler) createScan(mo mediaObject) (blob.Ref, error) {
	...
	// give it content
	if err := h.setAttribute(pn, nodeattr.CamliContent, mo.content.String()); err != nil {
...
func (h *handler) createDocument(doc document) (blob.Ref, error) {
	...
	for pageNumber, pageRef := range doc.pages {
		if err := h.setAttribute(pn, fmt.Sprintf("camliPath:%d", pageNumber), pageRef.String()); err != nil {
			return pn, fmt.Errorf("could not set document %v page %d to scan %q: %v", doc.blobRef, pageNumber, pageRef, err)
		}
	}
...
func (h *handler) persistDocAndPages(newDoc document) (blob.Ref, error) {
	...
	for _, page := range newDoc.pages {
		if err := h.setAttribute(page, "document", newDoc.blobRef.String()); err != nil {
			return br, fmt.Errorf("could not update scan %v with %v:%v", page, "document", newDoc.blobRef)
		}
	}
	return pn, nil
}
